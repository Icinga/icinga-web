			...
			Ext.apply(grid_config, {
				...
				applyState: function(state) {
					...
					if (state.store_origin_params) {
						...
						lmg.applyParamsToStore(store.originParams, store) && (!reload && (reload=true));
					}
					
					if (state.filter_params) {
						this.filter_params = state.filter_params;
						lmg.applyParamsToStore(this.filter_params, store) && (!reload && (reload=true));
					}
...
//					log(store.originParams);
		...
		this.grid =  new AppKit.Ext.grid.GridPanel(grid_config);
		...
		this.grid.on('render', function() {
			this.grid.getStore().on('datachange', function() {
				this.grid.getStore().on('datachange', bubbleEvent);
				bubbleEvent();
			}, this, { single: true });
		...
		this.applyEventsToGrid(this.grid);
