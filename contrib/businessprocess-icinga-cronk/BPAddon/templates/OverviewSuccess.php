...
Cronk.util.initEnvironment(<?php CronksRequestUtil::echoJsonString($rd); ?>, function() {
    ...
    var bpTreeGrid = Ext.extend(Ext.ux.tree.TreeGrid,{
        ...
        applyState: function(state) {
            ...
            this.autoRefresh = true;
            if(typeof(state.autoRefresh) !== undefined) {
                this.autoRefresh = state.autoRefresh;
            }
            var _this = this;
            (function() {
                _this.toggleAutorefresh(null,state.autoRefresh,true);
            }).defer(200);
            if(!state.filters)
                return null;
    ...
    var bpGridList = new bpTreeGrid({
        ...
        toggleAutorefresh: function(checkItem, checked, noStateupdate) {
                    ...
                    items: [{
                        ...
                        listeners: {
                            afterrender: function(c) {
                                var grid = c.ownerCt.ownerCt.ownerCt;
                                c.setChecked(grid.autoRefresh,true);

                                c.container[grid.autoRefresh ? "addClass" : "removeClass"](
                                    "x-menu-item-checked"
                                );
                            },
                            scope :this
                        ...
                        checked: false,
                        id: this.stateuid+'bp_BtnAutoRefresh',
                        checkHandler: function(c,toggled) {
                            c.ownerCt.ownerCt.ownerCt.ownerCt.toggleAutorefresh(c,toggled)
                        }
                    }]
                },
                scope: this
